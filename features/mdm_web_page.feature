Feature: Mdm::WebPage REST API

  As a client
  I want to get web_page data
...
#
  Scenario: Valid GET#index
    ...
    And   the web_site has web_pages
    When  the client makes a GET#index request for web_pages
    Then  the status code is 200
    And   it returns all web_pages for the web_site
  ...
  Scenario: GET#index with a service that does NOT exist
    ...
    And   a web_site that has web_pages
    When  the client makes a GET#index request for web_pages
    Then  the status code is 404
...
#
  Scenario: Valid GET#show
    ...
    And   the host has a service
    And   the service has a web_site
    And   the web_site has a web_page
    When  the client makes a GET#show request for web_page
    Then  the status code is 200
    And   it returns the web_page
  ...
  Scenario: GET#show with a service that does NOT exist
    Given a workspace that has a host
    And   a service that does NOT exist
    And   a web_site that has a web_page
    When  the client makes a GET#show request for web_page
    Then  the status code is 404
